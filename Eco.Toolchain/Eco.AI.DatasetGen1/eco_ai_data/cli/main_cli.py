...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="eco-ai-data")
    ...
    parser.add_argument(
        "--context",
        action="store_true",
        help="Include context field in generation samples. If OpenAI is enabled, context is summarized from function body.",
    )
    parser.add_argument(
        "--max-qa-pairs",
        type=int,
        default=None,
        metavar="N",
        help="Per source file: stop after N generation samples (for quick tests). Omit for no limit.",
    )
    ...
    report.add_argument("report_md")
    report.add_argument("--repo", default="")

    qa_flatten = sub.add_parser(
        "qa-flatten",
        help="Flatten pipeline JSONL entries into question/context/answer lines (context may be empty).",
    )
    qa_flatten.add_argument("input_jsonl", help="Full pipeline JSONL (e.g. module_full_pipeline.jsonl)")
...
def main() -> None:
    ...
    if args.command == "analyze":
        default_name = "dataset.jsonl" if args.output_format == "jsonl" else "dataset.json"
        output_json = args.output_json or str(Path(args.output_dir) / default_name)
        output_md = args.output_md or str(Path(args.output_dir) / "report.md")
    ...
    if args.command == "export":
        if not args.repo:
            raise SystemExit("Use --repo with export command.")
        pipeline.analyze(args.repo)
        path = pipeline.export(args.output_json)
        print(str(path))
        return
    if args.command == "report":
        if not args.repo:
            raise SystemExit("Use --repo with report command.")
        pipeline.analyze(args.repo)
        path = pipeline.report(args.report_md)
        print(str(path))
        return
