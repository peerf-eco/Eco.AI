...
ATTACH_MAX_CONTENT = 25_000_000     # hard cap on any single attachment's bytes
                                    # written/copied to disk (server-side; the
                                    # client-side 5 MB paste cap is bypassable)
...
3. For each item compute how it reaches the agent, preferring the **most token-efficient** option that still guarantees availability:
   ...
   - **text + `path` outside `project_dir`:** decide via `stat().st_size` FIRST and only `read_bytes()` when actually inlining — never read a file twice just to copy it. If `len ≤ ATTACH_INLINE_LIMIT` → inline; else copy to `att_dir/name` and reference the copy.
   ...
   - **Size caps:** every item is checked against `ATTACH_MAX_CONTENT` before any disk write/copy — oversized items are skipped with a warning (protects against a crafted WS message writing gigabytes).
   - **Name collisions:** written names are de-duplicated inside `att_dir` (`a.txt` → `a-2.txt`, …) so two attachments sharing a basename (e.g. a mention `src/a.txt` plus a pasted `a.txt`) can never clobber each other. Dedupe happens BEFORE building prompt lines so the emitted `read(path='.eco-attachments/…')` hints always name the file that was actually written.
   - **Idempotent copies:** session-scoped attachments are re-sent on EVERY `user_request`; skip the disk copy when an identical file (size + mtime) already sits at the destination. Pasted content, by contrast, is always (re)written — a size-only staleness check could keep stale bytes when a re-send carries different content of equal length, and the content is already in memory.
4. Build block:
   ```
   ...
   - <name> [text, N bytes] — inline:
     <content or first 4KB + "…(truncated, read full via read(path='.eco-attachments/<deduped-name>'))">
   - <name> [text, N bytes] — path-only: read(path='<relpath or .eco-attachments/<deduped-name>')
   - <name> [image] — visual reference at .eco-attachments/<deduped-name>
   ```

### 3.6 Inject block into every seed (server.py)
In `chat_endpoint`, after `project_dir` is finalized (post worktree) and before mode dispatch, compute:
```python
attached_block = _build_attached_block(payload.get("attached_files"), project_dir)
```
Then insert `attached_block` between the workspace header and the request in every seed — the volatile attachment content must come AFTER the stable `_workspace_header` prefix, never before it:
- **AUTO chat-reply branch** (the intent gate classified the message as plain CHAT): `_chat_reply` receives `attached_ctx=attached_block` and prepends it to the user message, so attached context is not silently dropped on questions.
- one-shot seed (test/review/code/plan): `workspace + attached_block + user_req`
- AUTO initial planner seed: `workspace + attached_block + user_req`
- planner-retry seed: `workspace + attached_block + (user_req + feedback)`
- coder seed: `workspace + attached_block + approved_plan_md + scaffold_note`
