def test_success_tool_returns_command_to_done():
    ...
    result = success.invoke({"args": {"summary_md": "all green"}, "name": "success", "type": "tool_call", "id": "test_call_id"})
...
def test_back_to_code_returns_command_to_coding_with_increment():
    ...
    result = back.invoke({"args": {"feedback_md": "## Stage: build\n## Status: FAIL\n## Errors\n- a.c:1: bad"}, "name": "back_to_code", "type": "tool_call", "id": "test_call_id"})
    assert isinstance(result, Command)
...
def test_back_to_code_at_max_iterations_marks_done():
    ...
    back = next(t for t in tools if t.name == "back_to_code")
    result = back.invoke({"args": {"feedback_md": "## Stage: test\n## Status: FAIL"}, "name": "back_to_code", "type": "tool_call", "id": "test_call_id"})
    assert result.update["phase"] == "done"
